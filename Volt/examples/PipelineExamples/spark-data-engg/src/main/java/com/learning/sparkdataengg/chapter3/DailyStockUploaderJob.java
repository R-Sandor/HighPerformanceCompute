...
public class DailyStockUploaderJob {
  ...
  public static void main(String[] args) {
    ...
    // System.setProperty("hadoop.home.dir", "/home/rsandor/Tools/hadoop-3.3.6/");

    // Set logger levels so we are not going to get info messages from underlying
    // libraries
  ...
  private static void uploadStock(String startDate, String endDate, String warehouseId) {
    ...
    try (
      ...
      // there will be errors that the process cannot delete the data or temp
      // directories
      ...
      String boundsQuery = "SELECT min(ID) as MIN_ID ,max(ID) as MAX_ID " +
          "FROM item_stock " +
      ...
      ResultSet rsBounds = warehouseConn
          .createStatement()
      ...
      System.out.println("Spark session sucessful");

      String stockQuery = "SELECT ID, date_format(STOCK_DATE,'%Y-%m-%d') as STOCK_DATE," +
      ...
      Dataset<Row> stockDF = spark.read()
          .format("jdbc")
